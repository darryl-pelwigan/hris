    ...
    $(document).ready(function() {
        ...
        $.fn.checkType = function() {
         ...
         /* autofill function*/
            ...
            $.fn.autofill = function(type,studtype){
                     $.ajax({
                        type: "POST",
                        url: APP_PATH + "registration/auto_fill",
                        data: {
                            type : type,
                            studtype : studtype
                        },
                        dataType: "json",
                        success: function(data) {
                         if(String(data[0]["birthdate"])!="null"){
                                var res = data[0]["birthdate"].split("/");
                                $("select[name='year']").html('<option value="'+res[2]+'">'+res[2]+'</option>');
                            }
                         
                            $("input[name='lname']").val(data[0]["lastname"]);
                            $("input[name='fname']").val(data[0]["firstname"]);
                            $("input[name='mname']").val(data[0]["middlename"]);
                            $("input[name='po_birth']").val(data[0]["birthplace"]);
                            $("select[name='sex']").html('<option value="'+data[0]["Gender"]+'">'+data[0]["Gender"]+'</option>');
                            $("input[name='cphone']").val("09101234567");
                             var mail_add = Math.random().toString(8).substring(7,14);
                             if(String(data[0]["emailadd"])!="null"){
                                $("input[name='emailadd']").val("test_"+data[0]["emailadd"]);
                                $("input[name='checkemailadd']").val("test_"+data[0]["emailadd"]);
                            }else{
                                
                                $("input[name='emailadd']").val("test"+mail_add+"@mail.com");
                                $("input[name='checkemailadd']").val("test"+mail_add+"@mail.com");
                            }
                            $("input[name='c_add_hn']").val(mail_add);
                            $("input[name='c_add_sn']").val(mail_add);
                            $("input[name='c_add_b']").val(mail_add);
                            $("input[name='c_add_m']").val(mail_add);
                            $("input[name='c_add_p']").val(mail_add);
                            
                            $("input[name='famBG_mom']").val("famBG_mom_"+mail_add);
                            $("input[name='famBG_mom_occupation']").val("famBG_mom_occupation_"+mail_add);
                            $("input[name='famBG_mom_tel']").val("09101234567");
                            $("input[name='famBG_mom_cphone']").val("09101234567");

                             $("input[name='famBG_dad']").val("famBG_dad"+mail_add);
                            $("input[name='famBG_dad_occupation']").val("famBG_dad_occupation"+mail_add);
                            $("input[name='famBG_dad_tel']").val("09101234567");
                            $("input[name='famBG_dad_cphone']").val("09101234567");

                             $("input[name='famBG_guardian']").val("famBG_guardian"+mail_add);
                             $("input[name='famBG_gurdian_tel']").val("09101234567");
                             $("input[name='famBG_gurdian_cphone']").val("09101234567");

                             $("textarea[name='elem_educ']").val("elem_educ_"+mail_add);
                             $("textarea[name='elem_educ_loc']").val("elem_educ_loc"+mail_add);
                             $("textarea[name='high_educ']").val("high_educ"+mail_add);
                             $("textarea[name='high_educ_loc']").val("high_educ_loc"+mail_add);
                        }
                    });
            };
        ...
        $.fn.checkFather = function(obj) {
         ...
         $.fn.cstatus = function(){
          ...
          $.fn.sent_registration = function (){
              var oading='<div class="modal fade" tabindex="-1" role="dialog" id="loading"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-body">loading</div></div></div></div>';
               var form_data = $("#regform").serialize();
               var nationality = $("#nationality").val();
               var citizenship = $("select[name='citizenship']").val();
                $('#loading').remove();
                  $("#viewSubmitted").append(oading);
                  $('#loading').modal("show");
               $.ajax({
                type: "POST",
                url: APP_PATH + "registration/submit_test",
                data: form_data+"&nationality="+nationality+"&citizenship="+citizenship,
                dataType: "html",
                success: function(data) {
                    $('#loading').modal("hide");
                  $('#registration_verify').remove();
                  $("#viewSubmitted").append(data);
                  $('#registration_verify').modal("show");
                  $('#loading').remove("hide");
                }
            });
          };
