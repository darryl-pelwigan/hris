...
class Export_mpdf extends MY_Controller {
	...
	public function types(){
		...
		if($data['check_sched'][0]["type"]!='Internship'){

			if($data['type']!='f'){

					if($data['check_sched']['labunits']>0 && $data['check_sched']['lecunits']>0){
						$comp_type_module="sched_record_list_".$comp_type[0]->comp_type;
						$this->load->module('sched_record2/'.$comp_type_module);

						if(substr($data['type'] ,0,1)=='t' || substr($data['type'] ,1,1)=='t')
							$type_lec= (substr($data['type'] ,0,1)=='l'?substr($data['type'] ,1,2):$data['type']);
						else
							$type_lec= (substr($data['type'] ,0,1)=='l'?substr($data['type'] ,1,2):$data['type']);

						$type_lab= substr($data['type'] ,0,1)=='l'?$data['type']:'l'.$data['type'];
						$data['grades']=$this->{$comp_type_module}->table( $settings , $data['labunits'] ,  $type_lec , $data['sched_id'] , $data['check_sched'] ,  $data['teacher_id'] , TRUE);
						$data['grades2']=$this->{$comp_type_module}->table( $settings , $data['labunits'] , $type_lab , $data['sched_id'] , $data['check_sched'] ,  $data['teacher_id'] , TRUE);
						// $data['grades3']['table']='';
						$data['grades3']=$this->sched_total_leclab->get_total_leclab($settings ,$data['check_sched']['labunits'], $data['labunits'] , $type_lec  , $data['sched_id'] , $data['check_sched'] ,  $data['teacher_id']);
					}else{
						$comp_type_module="sched_record_list_".$comp_type[0]->comp_type;
						$this->load->module('sched_record2/'.$comp_type_module);
						$data['grades']=$this->{$comp_type_module}->table( $settings , $data['labunits'] ,  $data['type'] , $data['sched_id'] , $data['check_sched'] ,  $data['teacher_id'] , TRUE);
						$data['grades2']['table']='';
						$data['grades3']['table']='';
					}
			}else{
				if($comp_type[0]->comp_type == 'fgradeonly'){
					$this->load->module('sched_record2/sched_record_list_fgradeonly');
					$data['grades']=$this->sched_record_list_fgradeonly->table($settings , $data['labunits'] ,  $data['type'] , $data['sched_id'] , $data['check_sched'] ,  $data['teacher_id'] , TRUE);
					$data['grades2']['table']='';
					$data['grades3']['table']='';
				}else{
					$data['grades']=$this->sched_finalgrade_record->get_finalgrade($settings , $data['sem'], $data['labunits'] ,  $data['type'] , $data['sched_id'] , $data['check_sched'] ,  $data['teacher_id'] , TRUE);
					$data['grades2']['table']='';
					$data['grades3']['table']='';
				}
			}
				...
				$data['grades']=$this->sched_intern_record->get_intern_record( $settings , $data['labunits'] ,  $data['type'] , $data['sched_id'] , $data['check_sched'] ,  $data['teacher_id'] , TRUE);
				$data['grades2']['table']='';
				$data['grades3']['table']='';
		}
