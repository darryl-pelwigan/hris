...
var prcnt_exm =Number($('#exam_prcnt').val());
     ...
     $.fn.computeCs = function(studentid,sched_id,type,cs_id,cs_i_id,c){
        ...
        var cs_id_list =($('#cs_id_list_x-'+type).val());
        var cs_i_id_list =($('#cs_id_list_'+sched_id+'-'+cs_id+'-'+type).val());
        var array_id = cs_id_list.split(',');
        var array_i_id = cs_i_id_list.split(',',2);

        var trans_cs = JSON.parse($('#cs_nsub_trans_'+cs_id).val());
        var sub_percent=Number($('#cs_'+sched_id+'-'+type+'-'+cs_id+'-prcnt').val());
            sub_percent = sub_percent/100;
            if((parseInt(inputed)>parseInt(maxx) || $.fn.checkInput(inputed,maxx.length)===false) && inputed!='' ){
                 ...
                 $.fn.computeCs(studentid,sched_id,type,cs_id,c);
                    ...
                    var total_sub = 0;
                    array_i_id.forEach(function(el){
                        if(el!=''){
                             var items_s = Number($('#cs_score_'+sched_id+'_'+cs_id+'_'+el+'_'+studentid+'-'+type+c).val());
                                 total_sub=total_sub+items_s;
                        }
                    });
                    var transmu = (trans_cs[total_sub]);
                    var cs_prcnt = transmu*sub_percent;
                        cs_prcnt = cs_prcnt.toFixed(2);
                    var total_cs = 0;
                    array_id.forEach(function(el){
                        if(el!=''){
                             var cs_prcnt_c = Number($('#cs_score'+'_'+studentid+'_'+sched_id+'_'+el+'-'+type+'-prcnt').text());
                                 total_cs = total_cs+cs_prcnt_c;
                        }
                    });
                    total_cs = total_cs.toFixed(2);
                    var total_prcnt_cs = total_cs*prcnt_cs;
                        total_prcnt_cs = total_prcnt_cs.toFixed(2);
                    var urlx ="<?=base_url('UpdateStudentCS_NSUB')?>";
                    var data_cs_array={};
                    ...
                    $('#cs_score'+'_'+studentid+'_'+sched_id+'_'+cs_id+'-'+type+'-total').text(total_sub);
                    $('#cs_score'+'_'+studentid+'_'+sched_id+'_'+cs_id+'-'+type+'-trn').text(transmu);
                    $('#cs_score'+'_'+studentid+'_'+sched_id+'_'+cs_id+'-'+type+'-prcnt').text(cs_prcnt);
                    ...
                    $('#exam_scorefinal_'+studentid+'_'+sched_id+'-'+type+' span').text(grade);
                    console.log(grade);
                   if(grade<75){
                    ...
                    $(this).attr('onblur','$(this).sent_data_cs("'+studentid+'","'+sched_id+'","'+type+'","'+cs_id+'","'+cs_i_id+'","'+inputed+'","'+c+'","'+urlx+'")');
                    return (data_cs_array);
            }
     };

     $.fn.sent_data_cs = function(studentid,sched_id,type,cs_id,cs_i_id,inputed,c,urlx){
                        $.ajax({
                                        type: "POST",
                                        url: urlx,
     ...
     $.fn.computeExam = function(studentid,exam_id,sched_id,type){
        ...
        var trans = JSON.parse($("#transmutation_json_exam").val());
            if((parseInt(inputed)>parseInt(maxx) || $.fn.checkInput(inputed,maxx.length)===false) && inputed!='' ){
                ...
                var equiv=(inputed=='') ? '': (trans[parseInt(inputed)]);
                ...
                var finalx = (Number(cs)+Number(prcnt));
                 ...
                 $('#exam_scorefinal_'+studentid+'_'+sched_id+'-'+type+' span').text(final);
