...
/**
 * continous computation of grades
 *
 *
 */
...
$("#tpl_more").on('click',function(){
     ...
     tr_count = tr_count+1;
       var add_items =      '   <tr class="cs_adding_wc">'+
                            '   <td>'+tr_count+'</td>'+
                            '   <td><div class="input-group col-sm-10 col-sm-offset-1"><input type="text" class="form-control" id="cs_desc_'+tr_count+'"><span class="dspl-hide" id="cs_desc_i_'+tr_count+'"></span></div></td>'+
                            '   <td><div class="input-group col-sm-7 col-sm-offset-1"><input type="number" min="5" max="1000" class="form-control"  id="cs_items_'+tr_count+'"><span id="cs_items_i_'+tr_count+'" class="dspl-hide"></span></div></td>'+
                            '   <td>'+
...
$.fn.updateCSitems = function(tr_index,cs_id){
    ...
    if(cs_desc==="" || cs_items==="" || cs_date===""){
        alert("NOT COMPLETE");
          ...
          $.ajax({
                                type: "POST",
                                url: "<?=base_url('sched_record2/sched_cs_record_contg/update_sched_cs')?>",
                                data : {
                                        cs_id     : cs_id,
                                        cs_desc   : cs_desc,
                                        cs_items  : cs_items,
...
$.fn.saveCSitems = function(tr_index){
    ...
    if(cs_desc==="" || cs_items==="" || cs_date===""){
        alert("NOT COMPLETE");
          ...
          $.ajax({
                                type: "POST",
                                url: "<?=base_url('sched_record2/sched_cs_record_contg/create_sched_cs')?>",
                                data : {
                                        cs_desc   : cs_desc,
                                        cs_items  : cs_items,
                                        cs_date   : cs_date,
                                    ...
                                    if(data!=0){
                                        ...
                                        $("#cs_date_i_"+tr_index).text(cs_date);
                                         ...
                                         if(tr_index>0){
                                            ...
                                            $.fn.getViewCsCONTG(type,sched_id);
                                        }
                                    }
                                }
                    });
...
//
 $.fn.viewCSitems = function(teacherId,csid,schedid,type,desc) {
    ...
    $('#classStandingModalViewCsItems #AddCsItems').attr('onclick','$(this).AddCsItems("'+teacherId+'","'+csid+'","'+schedid+'","'+type+'","'+desc+'");');
       $.ajax({
                            type: "POST",
                            url: "<?=base_url('sched_record2/sched_cs_record_contg/view_cs_items')?>",
                            data : {
                                    cs_id : csid,
 ...
 $.fn.deleteCSitems = function(tr_index,cs_id){
    ...
    $.ajax({
                            type: "POST",
                            url: "<?=base_url('sched_record2/sched_cs_record_contg/delete_sched_cs')?>",
                            data : {
                                    cs_id : cs_id,
                                    sched_id : sched_id,
                                    type :  $('#_this_type').val()
