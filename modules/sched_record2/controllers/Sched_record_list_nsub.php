...
class Sched_record_list_nsub extends MY_Controller {
	...
	public function table( $settings ,  $labunits ,  $type , $sched_id , $check_sched , $teacher_id , $pdf=FALSE ){
		...
		$data2['cs']=$this->sched_cs_record_model_nsub->get_cs(  $sched_id ,$teacher_id ,$type);
		$data2['cs_items_total']=$this->sched_cs_record_model_nsub->get_cs($sched_id , $teacher_id , $type,'SUM(items) as items_sum');
		...
		$data3['trans_exam']=$transmutation_exam;
	...
	public function table_th($lecunits, $labunits , $teacher_id ,$type , $sched_id ,$type_d , $cs , $exam  , $pdf){
			...
			if($cs['cs']['cs_num']>0  ){
				if($cs['cs']['cs_num']>0){
					for ($c=0;$c<count($cs['cs']["cs_query"]) ; $c++) {
						$cs_id_list_x .=$cs['cs']["cs_query"][$c]->id.',';
						$cs_items=$this->sched_cs_record_model_nsub->get_cs_items($sched_id,$teacher_id,$cs["cs"]["cs_query"][$c]->id,'id');
							$cs_items_count=$cs_items_count+$cs_items['cs_num'];
					}
				}
				$rowspan="rowspan='3'";
				$rowspan2="rowspan='2'";
			...
			$data['th'] = "<tr >
								...
								<th   ".$colspan." >
													Class Standing
													<input type='hidden' name='cs_count-$type' id='cs_count-$type' value='".$cs['cs']['cs_num']."' />
													<input type='hidden' name='cs_id_list_x-$type' id='cs_id_list_x-$type' value='".$cs_id_list_x."' />
								</th>
								<th >Total</th>
	        ...
	        $data['th'] .= "<tr >";
					...
					if($cs['cs']['cs_num']>0){
						for ($c=0;$c<count($cs['cs']["cs_query"]) ; $c++) {
							...
							$cs_items=$this->sched_cs_record_model_nsub->get_cs_items($sched_id,$teacher_id,$cs["cs"]["cs_query"][$c]->id,'id,items');
							$cs_sum_items=0;
							for($x=0;$x<$cs_items['cs_num'];$x++){
								$cs_sum_items=$cs_sum_items+$cs_items['cs_query'][$x]->items;
							}
							$data['th'] .= ' <th  colspan="'.(3+$cs_items['cs_num']).'" >
      			...
      			$data['th'] .= "</tr>";
				      if($cs['cs']['cs_num']>0){
				      	$total_cs_sum= 0 ;
				      			      $data['th'] .= "<tr >";
								for ($c=0;$c<count($cs['cs']["cs_query"]) ; $c++) {
									$cs_id=$cs["cs"]["cs_query"][$c]->id;
									$cs_items=$this->sched_cs_record_model_nsub->get_cs_items($sched_id,$teacher_id,$cs_id,'id,items,title,date_updated');
									$cs_sum_items=0;
									$cs_id_list ='';
									for($x=0;$x<$cs_items['cs_num'];$x++){
										$cs_id_list .=$cs_items['cs_query'][$x]->id.',';
										$cs_sum_items=$cs_sum_items+$cs_items['cs_query'][$x]->items;
										$data['th'] .= '<th  > '.$cs_items['cs_query'][$x]->title.' ('.$cs_items['cs_query'][$x]->items.')
														<input type="hidden" id="max_cs_'.$sched_id.'_'.$cs["cs"]["cs_query"][$c]->id.'_'.$cs_items['cs_query'][$x]->id.'-'.$type.'" value="'.$cs_items['cs_query'][$x]->items.'" />
														<img src="'.base_url('assets/images/icons/CSV.png').'" onclick="$(this).importDataCs(\''.$teacher_id.'\',\''.$sched_id.'\',\''.$cs["cs"]["cs_query"][$c]->id.'\',\''.$cs_items['cs_query'][$x]->id.'\',\''.$type.'\',\''.$cs["cs"]["cs_query"][$c]->description.'\',\''.$cs_items['cs_query'][$x]->title.'\',\''.date("M d , Y h:i:s A",strtotime($cs_items['cs_query'][$x]->date_updated)).'\');" width="20" />
														</th>';
						      		}
						      		$trans_cs=$this->transmutation->create_json($cs_sum_items,$type_d['prcnt']);
						      		$data['th'] .= "<th>Total <br/>($cs_sum_items)
						      							<input type='hidden' name='cs_id_list$sched_id-$type' id='cs_id_list_$sched_id-$cs_id-$type' value='$cs_id_list' />
														<input type='hidden' name='cs_nsub_trans_".$cs["cs"]["cs_query"][$c]->id."'  id='cs_nsub_trans_".$cs["cs"]["cs_query"][$c]->id."' value='".json_encode($trans_cs)."' />
						      						</th>";
	...
	public function student_cs_score($lecunits , $labunits , $teacher_id , $type , $sched_id ,$type_d , $studentid ,  $cs ,  $exam  , $pdf ){
		...
		if($cs['cs']['cs_num']>0){
			...
			for($x=0;$x<$cs['cs']['cs_num'];$x++){
				...
				$cs_items=$this->sched_cs_record_model_nsub->get_cs_items($sched_id,$teacher_id,$cs_id);
				#$max=$cs['cs']['cs_query'][$x]->items;
				$prcnt=$cs['cs']['cs_query'][$x]->percent;
				$score_total = 0;
				$scorexx ='';
				$cs_items_total=0;
				for($c=0;$c<$cs_items['cs_num'];$c++){

					$max=$cs_items['cs_query'][$c]->items;
					$cs_items_total=$cs_items_total+$max;
					if(!$this->_check_score($this->sched_cs_record_model_nsub->get_student_cs( $studentid , $cs_id ,$cs_items['cs_query'][$c]->id, $sched_id, $type , 'score' ),'score') && $pdf==FALSE){
							$scorexx .='<td><input type="number" name="cs_score_'.$sched_id.'_'.$cs_id.'_'.$cs_items['cs_query'][$c]->id.'_'.$studentid.'-'.$type.$x.'" id="cs_score_'.$sched_id.'_'.$cs_id.'_'.$cs_items['cs_query'][$c]->id.'_'.$studentid.'-'.$type.$x.'" min="0" max="'.$max.'" class="form-control inp-sm text-center empty-cs " onblur=""  oninput="$(this).computeCs(\''.$studentid.'\',\''.$sched_id.'\',\''.$type.'\',\''.$cs_id.'\',\''.$cs_items['cs_query'][$c]->id.'\',\''.$x.'\');" /></td>';
					}elseif($this->sched_cs_record_model_nsub->get_student_cs( $studentid , $cs_id ,$cs_items['cs_query'][$c]->id, $sched_id, $type , 'score' )){
							$score =	$this->sched_cs_record_model_nsub->get_student_cs( $studentid , $cs_id ,$cs_items['cs_query'][$c]->id, $sched_id, $type , 'score' )[0]->score;
							$score_total = $score_total+$score;
							$scorexx .='<td  ondblclick="$(this).ena_edit_cs(\''.$studentid.'\',\''.$sched_id.'\',\''.$type.'\',\''.$cs_id.'\',\''.$cs_items['cs_query'][$c]->id.'\',\''.$x.'\');" ><input type="hidden" name="cs_score_'.$sched_id.'_'.$cs_id.'_'.$cs_items['cs_query'][$c]->id.'_'.$studentid.'-'.$type.$x.'" id="cs_score_'.$sched_id.'_'.$cs_id.'_'.$cs_items['cs_query'][$c]->id.'_'.$studentid.'-'.$type.$x.'" min="0" max="'.$max.'" class="form-control inp-sm text-center empty-cs " onblur=""  oninput="$(this).computeCs(\''.$studentid.'\',\''.$sched_id.'\',\''.$type.'\',\''.$cs_id.'\',\''.$cs_items['cs_query'][$c]->id.'\',\''.$x.'\');" value="'.$score.'" />
									<span>'.$score.'</span></td>';
				...
				$trans_cs=$this->transmutation->create_json($cs_items_total,$type_d['prcnt']);
				$cs_trn = $trans_cs[$score_total];
			...
			for($x=0;$x<$exam['exam']['exam_num'];$x++){
				...
				if($this->_check_score($this->sched_record_model->get_exam_score($lecunits , $labunits , $teacher_id , $studentid , $sched_id , $type , $type_d[8]),$type_d[8]) ){
					$exam_prcnt= $exam['trans_exam'][$score] * ($type_d[2]);
					 $td .='<td id="exam_scoreprcnt_'.$studentid.'_'.$sched_id.'-'.$type.'">'.round($exam_prcnt,2) .'</td>';
