...
class Sched_cs_record_wc extends MY_Controller {
	...
	function __construct(){
     ...
     public function get_cs_record( $settings , $type , $sched_id , $check_sched , $teacher_id){
        ...
        $data['cs'] = $this->sched_cs_record_model_wc->get_cs($sched_id,$data['teacher_id'],$type);
	 ...
	 public function cs_tbody($sched_id,$type_d,$cs_list, $teacher_id){
		...
		if($cs_list['cs_num']>0){
			...
			for($x=0;$x<($cs_list['cs_num']);$x++){
				...
				$tbody['tr'] .='
					  <tr>
					  <td style="width:55px">'.$count++.'</td>
					  <td style="width:255px"><div class="input-group col-md-10 col-md-offset-1" ><input type="hidden" class="form-control " id="cs_desc_'.$x.'" value="'.$cs_q->title.'"   /><span id="cs_desc_i_'.$x.'" > '.$cs_q->title.'</span></div></td>
					  <td style="width:255px"><div class="input-group col-md-7 col-md-offset-1"><input type="hidden" min="5" max="1000" class="form-control" id="cs_items_'.$x.'" value="'.$cs_q->items.'" /><span id="cs_items_i_'.$x.'" > '.$cs_q->items.'</span></div></td>
					  <td style="width:255px">
							<div class=" dspl-hide input-group date datetimepicker1 col-md-7 col-md-offset-1" onclick="$(this).datePickershow('.$x.');" >
	 ...
	 public function create_sched_cs(){
		...
		$trans_status=$this->sched_cs_record_model_wc->create_sched_cs( $data );
			return $trans_status;
	}
	...
	public function delete_sched_cs(){
		...
		$teacher_id = $this->session->userdata('id');
		$trans_status=$this->sched_cs_record_model_wc->delete_sched_cs( $schedid,$teacher_id,$cs_id,$type );
			return $trans_status;
	}
	...
	public function update_sched_cs(){
		...
		if($this->session->userdata('id')==$teacher_id){
			$get_cs_item = $this->sched_cs_record_model_wc->get_cs_one( $cs_id,$sched_id,$teacher_id,$type );

			if($get_cs_item['cs_query'][0]->items>$cs_items){
				$trans_status=$this->sched_cs_record_model_wc->update_sched_cs_del_scores( $sched_id,$teacher_id,$type,$cs_id,$cs_desc,$cs_items,$cs_date );
			}else{
				$trans_status=$this->sched_cs_record_model_wc->update_sched_cs( $sched_id,$teacher_id,$type,$cs_id,$cs_desc,$cs_items,$cs_date );
			}
	...
	public function update_student_cs(){
		...
		if($this->sched_cs_record_model_wc->get_student_cs( $studentid,$cs_id,$sched_id,$type )){
			$trans_status=$this->sched_cs_record_model_wc->update_student_cs( $studentid,$cs_id,$cs_items,$sched_id,$type );
			echo $trans_status;
		}else{
			if($this->sched_cs_record_model_wc->insert_student_cs( $studentid,$cs_id,$cs_items,$sched_id,$type ))
				echo TRUE;
			else
				echo FALSE;
		}
	...
	public function view_cs_items(){
		...
		$sched_id = $this->input->post('sched_id', TRUE);
		$type = $this->input->post('type', TRUE);
		$data['items']=$this->sched_cs_record_model_wc->view_cs_items($teacher_id,$sched_id,$cs_id,$type);
	...
	public function add_cs_items(){
		...
		$cs_items=$this->sched_cs_record_model_wc->view_cs_items($teacher_id,$sched_id,$cs_id,$type,'SUM(items) as sum_items');
		...
		if($cs_items>400){
			...
			$this->sched_cs_record_model_wc->add_cs_items($data);
	...
	public function edit_cs_item(){
		...
		echo $this->sched_cs_record_model_wc->edit_cs_item($sched_id,$type,$cs_id,$title,$items);
	}
	...
	public function show_selected_template(){
		...
		$cs_tpl = $this->sched_cs_record_model_wc->get_cs_template($cs_tpl_id,'',$check_sched);
		$data='';
	...
	public function create_cs_template(){
		...
		if($check_sched[0]['lecunits']>0 && $check_sched['labunits'] == 0 ){
			...
			$trans_status=$this->sched_cs_record_model_wc->create_cs_template( $data );
	...
	public function import_data_csv(){
		...
		for($x=0;$x<$student_count;$x++){
			...
			if(!$this->sched_cs_record_model_wc->get_student_cs( $scores[$x]['studentid'],$cs_id,$sched_id,$type )){
				$trans_status .=$this->sched_cs_record_model_wc->insert_student_cs( $scores[$x]['studentid'],$cs_id,$scores[$x]['score'],$sched_id,$type );
			}else{
				$trans_status .=$this->sched_cs_record_model_wc->update_student_cs( $scores[$x]['studentid'],$cs_id,$scores[$x]['score'],$sched_id,$type );
			}
		}
