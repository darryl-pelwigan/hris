...
class Check_submitted extends MY_Controller {
    ...
    public function  get_all_subjects(){
        ...
        $table = [];
        $get_courses = $this->check_submitted_model->dept_courses( $data['teacher_dean'][0]->college_dept);
        $search['type'] = 's.course';
        foreach ($get_courses as $key => $course) {
            $search['value'] = $course->courseid;
            array_push($table,$this->set_se_tbody($sem,$sy,$search));
        }
        $data['table'] = $table;
        $data['view_content']='check_submitted/subject_list';
        $data['get_plugins_js']='check_submitted/plugins_js';
    ...
    public function set_se_tbody($sem,$sy,$search_text = ""){
        ...
        for($x=0;$x<count($data['aaData']);$x++){
            $countxy =$search_text["type"] ;
                    $enc_sched_id =     $data['aaData'][$x]->schedid;
            ...
            public function submitted(){
                ...
                if( $this->sched_grades_model->get_grade_review( $sched_id ) ){
                    ...
                    echo json_encode([0,'No grades has been submitted!.']);
                }
            }
            ...
            public function view_grades(){
                ...
                if($sched_id){
                        ...
                        $data["enc_sched_id"] =  $enc_sched_id;
                        ...
                        $data['view_content']='check_submitted/sched_grades_record';
                        $data['get_plugins_js']='check_submitted/sched_grades_js';
                ...
                $tfoot = '<tr><td colspan="3">DATE SUBMITTED</td>';
                    ...
                    if($data['check_sched'][0]['semester'] != 3){
                        if(isset($data['check_submitted'][0])){
                             if(  $data['check_submitted'][0]->requ_prelim != NULL   ){
                                 $dean_tfoot .=  '<td>
                                                <button class="btn btn-warning" onclick="$(this).approvedRequestUpdate(\'prelim\',\''.$data["enc_sched_id"].'\');" >APPROVE REQUEST UPDATE </button>
                                            </td>';

                            }elseif($data['check_submitted'][0]->submit_prelim != null && $data['check_submitted'][0]->checkdean_prelim == null){
                                 $dean_tfoot .=  '<td>
                                                <button class="btn btn-primary" onclick="$(this).approvedGRADES(\'prelim\',\''.$data["enc_sched_id"].'\');" >APPROVE PRELIM </button>
                                            </td>';
                            }else{
                                $nice_date_p = nice_dateX($data['check_submitted'][0]->checkdean_prelim,'M d, Y H:i');
                                 $dean_tfoot .=  '<td><strong class="date-approve">'.$nice_date_p .'</strong></td>';
                            }
                            ...
                            $dean_tfoot .=  '<td></td>';
                            $tfoot .=   '<td><strong class="btn btn-danger" >Not Submitted</strong></td>';
                    ...
                    if(isset($data['check_submitted'][0])){
                         if( ($data['check_submitted'][0]->submit_midterm != null && $data['check_submitted'][0]->checkdean_midterm == null)  || $data['check_submitted'][0]->requ_midterm != NULL ){

                                if(  $data['check_submitted'][0]->requ_midterm != NULL && $data['check_submitted'][0]->requ_midterm_remarks != NULL ){
                                 $dean_tfoot .=  '<td>
                                                <button class="btn btn-warning" onclick="$(this).approvedRequestUpdate(\'midterm\',\''.$data["enc_sched_id"].'\');" >APPROVE REQUEST UPDATE </button>
                                            </td>';
                                }elseif($data['check_submitted'][0]->submit_prelim != null && $data['check_sched'][0]['semester'] != 3 && $data['check_submitted'][0]->checkdean_prelim != null ){
                                    $dean_tfoot .=   '<td ><button class="btn btn-primary" onclick="$(this).approvedGRADES(\'midterm\',\''.$data["enc_sched_id"].'\');" >APPROVE MIDTERM </button></td>';
                                }elseif($data['check_submitted'][0]->submit_midterm != null && $data['check_sched'][0]['semester'] == 3 && $data['check_submitted'][0]->submit_midterm != null ){
                                    $dean_tfoot .=   '<td ><button class="btn btn-primary" onclick="$(this).approvedGRADES(\'midterm\',\''.$data["enc_sched_id"].'\');" >APPROVE MIDTERM </button></td>';
                                }else{
                                    $dean_tfoot .=   '<td>x</td>';
                                }
                                ...
                                $nice_date_m =  nice_dateX($data['check_submitted'][0]->checkdean_midterm,'M d, Y H:i');
                                $dean_tfoot .=  '<td><strong class="date-approve">'.$nice_date_m.'</strong></td>';
                                $tfoot .=   '<td>
                                                <strong class="date-approve">'.nice_dateX($data['check_submitted'][0]->submit_midterm,'M d, Y H:i').'</strong>
                                            </td>';
                    ...
                    if(isset($data['check_submitted'][0])){
                        if( ( $data['check_submitted'][0]->submit_tentative != null && $data['check_submitted'][0]->checkdean_tentative == null )  || $data['check_submitted'][0]->requ_tentative != NULL  ){
                            if(  $data['check_submitted'][0]->requ_tentative != NULL && $data['check_submitted'][0]->requ_tentative_remarks != NULL  ){
                             $dean_tfoot .=  '<td>
                                ...
                                $dean_tfoot .=   '<td><button class="btn btn-primary" onclick="$(this).approvedGRADES(\'tentative\',\''.$data["enc_sched_id"].'\');" >APPROVE TENTATIVE</button></td>';
                            }else{
                                $dean_tfoot .=   '<td></td>';
                            }
                             $tfoot .=   '<td>
                            ...
                            $registrar_tfoot .='<td></td>';
                             ...
                             $nice_date_t =  nice_dateX($data['check_submitted'][0]->checkdean_tentative,'M d, Y H:i');
                            $dean_tfoot .=  '<td><strong class="date-approve">'. $nice_date_t .'</strong></td>';
                            $tfoot .=   '<td>
                                            <strong class="date-approve">'.(  nice_dateX($data['check_submitted'][0]->submit_tentative,'M d, Y H:i')  ).'</strong>
                                        </td>';
                        ...
                        if( ($data['check_submitted'][0]->submit_final != NULL  && $data['check_submitted'][0]->checkdean_final == NULL  )  || $data['check_submitted'][0]->requ_final != NULL  ){
                            if(  $data['check_submitted'][0]->requ_final != NULL && $data['check_submitted'][0]->requ_final_remarks != NULL  && $data['check_submitted'][0]->checkregistrar_final == NULL ){
                             $dean_tfoot .=  '<td>
                                ...
                                $dean_tfoot .=   '<td colspan="2"><button class="btn btn-primary" onclick="$(this).approvedGRADES(\'final\',\''.$data["enc_sched_id"].'\');" >APPROVE FINAL</button></td>';
                            }else{
                            ...
                            $nice_date_f =  nice_dateX($data['check_submitted'][0]->submit_final,'M d, Y H:i');
                            $dean_tfoot .=  '<td colspan="1" ><strong class="date-approve">'.$nice_date_f.'</strong></td>';
                            $tfoot .=   '<td  colspan="2">
                                            <strong class="date-approve">'.( nice_dateX($data['check_submitted'][0]->submit_final,'M d, Y H:i')  ).'</strong>
                                        </td>';
                            $registrar_tfoot .= '<td  colspan="1" ><strong  class="date-approve">'.( nice_dateX($data['check_submitted'][0]->checkregistrar_final,'M d, Y H:i') ).'</strong></td>';
                        }
                    }else{
                            $dean_tfoot .=  '<td></td>';
                            $tfoot .=   '<td><strong class="btn btn-danger" >Not Submitted</strong></td>';
    ...
    public function submit_grade_review(){
        ...
        if($sched_id && type_grade($type)  ){
            ...
            if( $this->sched_grades_model->checkdean_grade_review( $sched_id, $dean ,$type ) ){
