...
class Check_submitted_sagso extends MY_Controller {
    ...
    public function  get_all_subjects(){
        ...
        $data['get_plugins_js']='check_submitted/sagso_plugins_js';
    ...
    public function set_se_tbody($sem,$sy,$search_text = ""){
        ...
        $data["table"] = '<hr /><table class="table table-bordered" id="dataTables-example"><tr><th>Assigned Teacher</th><th>Course No</th><th>Descriptive Title</th><th>Units</th><th>Days</th><th>Time</th><th>Room</th><th>Section</th><th>Enrolled</th><th>view grades</th></tr>';
        ...
        for($x=0;$x<count($data['aaData']);$x++){
            ...
            for($y=0;$y<count($data['aaData2']);$y++){
                ...
                if( $this->sched_grades_model->get_grade_review( $data['aaData2'][$y]->schedid ) ){
                    $get_grade_review = $this->sched_grades_model->get_grade_review( $data['aaData2'][$y]->schedid );
                    ...
                    $check_notify = $this->check_submitted_model->get_notify( $data['aaData2'][$y]->schedid );
                    if($check_notify){
                        if( $check_notify[0]->prelim != NULL || $check_notify[0]->midterm != NULL || $check_notify[0]->tentative != NULL ){
                            $view = '<button  class="btn btn-info btn-sm" disabled="" > notified</button>';
                        }else{
                            $view = '<button  class="btn btn-warning btn-sm" > not yet submitted</button>';
                        }
                    }else{
                         $view = '<button  class="btn btn-warning btn-sm" > not yet submitted</button>';
                    }
                        ...
                        <td class="text-center">'.($teacher).'</td>
                            <td>'.$data['aaData2'][$y]->courseno.'</td><td>'.$data['aaData2'][$y]->description.'</td><td>'.$data['aaData2'][$y]->totalunits.'</td><td>'.$data['aaData2'][$y]->days.'</td><td>'.$time.'</td><td>'.$data['aaData2'][$y]->room.'</td><td>'.$data['aaData2'][$y]->section.'</td>
                                <td> '.$count_studs.'
                                    </td>
            ...
            public function view_grades(){
                ...
                if($sched_id != 'undefined'){
                     // $this->sched_grades->gradesssss($sched_id,false);
                        ...
                        $data['view_content']='check_submitted/sched_grades_record_sagso';
                        $data['get_plugins_js']='check_submitted/sagso_sched_grades_js';
                    ...
                    redirect('ViewSubmittedGradesSAGSO');
                  }
            }
                    ...
                    if($data['check_sched'][0]['semester'] !== 3){
                     ...
                     if( ($data['check_submitted'][0]->submit_midterm != null && $data['check_submitted'][0]->checkdean_midterm == null)  || $data['check_submitted'][0]->requ_midterm != NULL ){
                        ...
                        if( ($data['check_submitted'][0]->submit_final != NULL  && $data['check_submitted'][0]->checkdean_final == NULL  )  || $data['check_submitted'][0]->requ_final != NULL  ){
                            ...
                            $dean_tfoot .=  '<td colspan="2" ><strong class="date-approve">'.$nice_date_f.'</strong></td>';
                            $tfoot .=   '<td  colspan="2">
                                            <strong class="date-approve">'.( nice_dateX($data['check_submitted'][0]->submit_final,'M d, Y H:i') ).'</strong>
                                        </td>';
                            $registrar_tfoot .= '<td  colspan="2" ><strong  class="date-approve">'.( nice_dateX($data['check_submitted'][0]->checkregistrar_final,'M d, Y H:i') ).'</strong></td>';
                        }
                $dean_tfoot .= ' </tr>';
                $registrar_tfoot .= ' </tr>';
    ...
    public function _check_login_sagso(){
